...
This method requires you to provide estimates of all relevant parameters. Some pertain to the stimuli, some can be taken from the literature, and some have to be estimated.
...
```{r Staircase1, include=TRUE,echo=FALSE}
ID = paste0("s",1:5)
...
```{r Staircase3, include=TRUE,echo=FALSE}
...
```{r Staircase4, include=TRUE,echo=FALSE}
...
```{r Staircase5, include=TRUE,echo=FALSE}
  Psychometric = Psychometric %>%
    mutate(
...
```{r Staircase6, include=TRUE,echo=FALSE}
  Psychometric = Psychometric %>%
    mutate(
...
```{r Staircase7, include=TRUE,echo=FALSE}
...
```{r Staircase8, include=TRUE,echo=FALSE}
Psychometric = Psychometric %>%
  mutate(
...
We again use an ANOVA to test whether the test model is better than the comparison model:
```{r Staircase13.6, include=TRUE}
anova(mod1,mod2)
summary(mod1)
```
...
SimulatePsychometricFunction_Staircase = function(ID, ConditionOfInterest, StandardValues, reps, PSE_Difference, JND_Difference, Mean_Standard, SD_Standard, SD_ResponseFunction){
  Psychometric = expand.grid(ID=ID, ConditionOfInterest=ConditionOfInterest, StandardValues=StandardValues, reps = reps)
...
for (i in NumbersOfSubjects){
  ...
  out <- replicate(nIterations, { #this function performs the number of iterations indicated above and stores the result for each in "out"
    Analyze_Pychometric_Precision(SimulatePsychometricFunction_Staircase(ID, ConditionOfInterest, StandardValues, reps, PSE_Difference, JND_Difference, Mean_Standard, SD_Standard, SD_ResponseFunction))})
...
for (i in NumbersOfSubjects){
  ...
  out2 <- replicate(nIterations, { #this function performs the number of iterations indicated above and stores the result for each in "out2"
    Analyze_Pychometric_Accuracy(SimulatePsychometricFunction_Staircase(ID, ConditionOfInterest, StandardValues, reps, PSE_Difference, JND_Difference, Mean_Standard, SD_Standard, SD_ResponseFunction))})
